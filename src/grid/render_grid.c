...
static void compute_cell_size(t_data *data, Cell *cell)
{
    ...
    int usable_h = data->terminal_max_y - (data->rows + 1);		//same for height

    if (usable_w < data->columns || usable_h < data->rows) // Safety check: if the usable area is smaller than the grid
    {
        cell->w = 2;
        cell->h = 1;
        return;
...
int render_grid(t_data *data, Cell *cell)
{
	...
	int need_h = data->rows * (cell->h + 1) + 1;
